...
for epoch in range(num_epochs):
    ...
    for audio, video, labels in train_loader:
        ...
        # 정확도 계산
        probabilities = torch.sigmoid(outputs)  # 로짓을 확률로 변환
        predicted = (probabilities > 0.5).float()  # 0.5 기준으로 이진 분류
    ...
    # 모델 저장
    model_save_path = os.path.join("/content/drive/MyDrive/AIX_DL_highlight_detector/checkpoint_BCE", f"./highlights_classifier{epoch+1}.pth")
